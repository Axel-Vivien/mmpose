...
ONLY unbiased videos - excluding true_positive and false_positive
"""
...
def categorize_videos_only_unbiased(data):
    """Categorize videos by their path, including only unbiased videos"""
    ...
    for video_path, video_data in data.items():
        ...
        if len(path_parts) >= 4:
            ...
            # Only include unbiased, exclude true_positive and false_positive
            if category == 'unbiased':
                categories[category][video_path] = video_data
            elif category in ['true_positive', 'false_positive']:
                excluded_count += 1
    
    print(f"Excluded {excluded_count:,} true_positive and false_positive videos")
    return categories

def create_batches_only_unbiased(categories, num_batches=8):
    """Split unbiased category into equal batches"""
    all_batches = []
    
    # Calculate videos per batch for unbiased category
    videos_per_batch = {}
    total_videos = 0
    ...
    # Distribute videos from unbiased category across batches
    for category, videos in categories.items():
...
def save_batch_files(batches, output_dir=".", prefix="video_batch_only_unbiased"):
...
def create_summary(batches, batch_filenames, original_filename="action_timespans.json"):
    ...
    summary = {
        "split_summary": {
            ...
            "excluded_categories": ["true_positive", "false_positive"],
            "included_categories": ["unbiased"],
...
def save_summary(summary, output_dir=".", filename="batch_summary_only_unbiased.json"):
...
def print_distribution_analysis(categories, batches):
    ...
    # Original distribution (only unbiased)
    print("\nOriginal distribution (only unbiased):")
...
def main():
    ...
    output_dir = "video_batches_only_unbiased"
    num_batches = 8
    
    # Load and process data
    data = load_action_timespans(input_file)
    categories = categorize_videos_only_unbiased(data)
    
    print(f"\nFound categories (only unbiased): {list(categories.keys())}")
    for category, videos in categories.items():
        print(f"  {category}: {len(videos):,} videos")
    
    # Create batches
    print(f"\nCreating {num_batches} batches...")
    batches = create_batches_only_unbiased(categories, num_batches)
    ...
    print(f"Summary saved as 'batch_summary_only_unbiased.json'")
    print(f"EXCLUDED: true_positive and false_positive videos")
    print(f"INCLUDED: unbiased videos only")
